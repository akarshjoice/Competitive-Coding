...
int counter = 0;
int countSubtreesWithSumX1(Node* root, int X)
{
	if(root == NULL) {
	    return 0;
	} 
	int lt_sum = countSubtreesWithSumX1(root->left, X);
	int rt_sum = countSubtreesWithSumX1(root->right, X);
	if((root->data + lt_sum + rt_sum) == X) {
	    counter+=1;
	}
	return (root->data + lt_sum + rt_sum);
}
